...
// NOTE: LLM calls are intentionally removed from the worker to avoid SSRF.
// All LLM preprocessing (optimize_for_tts, add_ssml_markup) should be performed in the client
// and the worker should only receive `input` or `raw_ssml`.
		...
		try {
			switch (url.pathname) {
				...
				case '/v1/audio/speech': {
					...
					// Preprocessing: worker expects either raw_ssml or plain input with optional prosody
